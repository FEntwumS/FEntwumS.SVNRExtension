...
begin
    ...
    s_wr_en <= i_wr_en and not w_full;  
    ...
    begin
        if rising_edge(i_clk) then
            if i_rst_sync = '1' then
                ...
                if (s_wr_en = '1' and i_rd_en = '0') then
                    r_FIFO_COUNT <= r_FIFO_COUNT + 1;
                elsif (s_wr_en = '0' and i_rd_en = '1') then
                    r_FIFO_COUNT <= r_FIFO_COUNT - 1;
                end if;
                ...
                if s_wr_en = '1' then
                    case r_WR_INDEX is
                        when 0 =>
                            r_FIFO_DATA(0) <= i_wr_data;
                        when 1 =>
                            r_FIFO_DATA(1) <= i_wr_data;
                        when 2 =>
                            r_FIFO_DATA(2) <= i_wr_data; 
                    end case;                                                       
                    --r_FIFO_DATA(r_WR_INDEX) <= "00000000";
                end if;
    ...
    p_WRITE : process (i_clk) is
        begin
            if rising_edge(i_clk) and s_wr_en = '1' then
    ...
    begin
        if rising_edge(i_clk) then
            if s_wr_en = '1' and w_FULL = '1' then
